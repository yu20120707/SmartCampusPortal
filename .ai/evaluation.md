...
- teacher score entry workflow has been added to existing `com.ruoyi.campus.academic`, `CampusAcademicMapper.xml`, `academic.js`, and `views/campus/academic/teacher.vue`
- teacher score API runtime smoke passed: teacher listed section `100` scores, updated student `100` from `91.5` to `92.5`, saw grade point `4.0`, then restored score to `91.5`
...
- teacher score build passed with `mvn clean install -pl ruoyi-admin -am -DskipTests`
- teacher score frontend addition passed with `NODE_OPTIONS=--openssl-legacy-provider npm run build:prod`
